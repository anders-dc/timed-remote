...
bool
ir_files(const char *dir_path, FuriString ***files, size_t *count)
{
	...
	*count = 0;
	...
	while (storage_dir_read(dir, &file_info, name_buf, sizeof(name_buf))) {
		if (!ir_is_file(&file_info, name_buf))
			continue;
...
static bool
ir_add_signal(IrSignalList *list, InfraredSignal *signal, const char *name)
{
	IrSignalItem *items;
	size_t new_capacity;
...
static bool
ir_is_file(const FileInfo *file_info, const char *name)
{
	size_t len;

	if ((file_info->flags & FSF_DIRECTORY) || name == NULL)
		return false;
