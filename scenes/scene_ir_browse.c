...
static void
pick_file(void *context, uint32_t index)
{
	TimedRemoteApp *app;
	...
	snprintf(
	    ...
	    "%s/%s",
	    IR_DIRECTORY,
	    furi_string_get_cstr(files[index]));
...
scene_browse_enter(void *context)
{
	...
	if (ir_files(IR_DIRECTORY, &files, &nfiles)) {
		if (nfiles == 0) {
			submenu_add_item(
			    app->submenu, "(No IR files found)", 0, NULL, NULL);
		} else {
			for (i = 0; i < nfiles; i++) {
				submenu_add_item(
				    app->submenu,
				    furi_string_get_cstr(files[i]),
				    i,
				    pick_file,
				    app);
			}
		...
		submenu_add_item(
		    app->submenu, "(Error reading directory)", 0, NULL, NULL);
	}
...
scene_browse_event(void *context, SceneManagerEvent event)
{
	...
	if (event.type != SceneManagerEventTypeCustom)
		return false;
	if (event.event != EVENT_FILE_SELECTED)
		return false;
