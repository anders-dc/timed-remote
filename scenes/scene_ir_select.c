...
static void
pick_signal(void *context, uint32_t index)
{
	TimedRemoteApp *app;

	app = context;
	if (signals == NULL || index >= signals->count)
		return;
	...
	strncpy(
	    app->sig,
	    furi_string_get_cstr(signals->items[index].name),
	    sizeof(app->sig) - 1);
...
scene_select_enter(void *context)
{
	...
	if (signals == NULL) {
		...
		if (signals->count == 0) {
			submenu_add_item(
			    app->submenu, "(No signals in file)", 0, NULL, NULL);
		} else {
...
scene_select_event(void *context, SceneManagerEvent event)
{
	...
	if (event.type != SceneManagerEventTypeCustom)
		return false;
	if (event.event != EVENT_SIGNAL_SELECTED)
		return false;
